...
function ExamplePanel({ context }: { context: PanelExtensionContext }): ReactElement {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // setTimer((prevCount) => prevCount + 1); // "correct way to update state"
      setTimer(Date.now());
    }, 200);
  ...
  return (
    <div style={{ padding: "1rem" }}>
      ...
      <h1>TIMER</h1>
      <p>{ new Date(timer).toString() }</p>
      <div>
        <a>{ new Date(timer).getHours() } </a>:
        <a>{ new Date(timer).getMinutes() } </a>:
        <a>{ new Date(timer).getSeconds() } </a>.
        <a>{ new Date(timer).getMilliseconds() } </a>
      </div>
